describe("Card rules:", function() {
    ...
    it("Only valid cards can start", function() {
        ...
        expect(kadi.RuleEngine.canStart(king)).toBe(false);

        var queen = kadi.hearts("Q");
        expect(kadi.RuleEngine.canStart(queen)).toBe(false);

        var five = kadi.hearts("5");
        expect(kadi.RuleEngine.canStart(five)).toBe(true);
    });
    ...
    it("Cards of the same suite can follow each other", function() {
        ...
        expect(kadi.RuleEngine.canFollow(random_spade_card, other_spade_card)).toBe(true);
        expect(kadi.RuleEngine.canFollow(other_spade_card,random_spade_card)).toBe(true);
        expect(kadi.RuleEngine.canFollow(other_spade_card,cant_follow)).toBe(false);
    });
    ...
    it("Cards of a different suite but same rank can follow each other", function() {
        ...
        expect(kadi.RuleEngine.canFollow(random_diamond,random_spade)).toBe(true);
    });
    ...
    it("An ace can follow any card", function() {
        ...
        expect(kadi.RuleEngine.canFollow(an_ace,any_card));
    });
    ...
    it("Any card can follow a joker", function() {
        ...
        expect(kadi.RuleEngine.canFollow(anyCard, j)).toBe(true);
        expect(kadi.RuleEngine.canFollow(j, anyCard)).toBe(true);
    });
    ...
    it("A card can be be in a move with another if they are the same rank", function() {
        ...
        expect(kadi.RuleEngine.canPlayTogetherWith(five_d, five_s)).toBe(true);
    });
    ...
    it("A card cannot be in a move with another if they are not the same rank", function() {
        ...
        expect(kadi.RuleEngine.canPlayTogetherWith(five_d, six_d)).toBe(false);
    });
    ...
    it("An ace can not be in a move a non ace card", function() {
        ...
        expect(kadi.RuleEngine.canPlayTogetherWith(five_d, ace)).toBe(false);
    });
    ...
    it("A King and a Queen cannot be in a move even if they are in the same rank", function() {
        ...
        expect(kadi.RuleEngine.canPlayTogetherWith(king_spades, queen_spades)).toBe(false);
    });
    ...
    it("A Queen can be in a move with Kings if the player reverses cancel each other out", function() {
        ...
        expect(kadi.RuleEngine.evaluateGroup(hand)).toBe(true);
        expect(kadi.RuleEngine.evaluateGroup([kadi.diamonds("K"), kadi.diamonds("Q")])).toBe(false);
        expect(kadi.RuleEngine.evaluateGroup([kadi.hearts("K"), kadi.diamonds("K"), kadi.diamonds("Q")])).toBe(true);
        expect(kadi.RuleEngine.evaluateGroup([kadi.hearts("K"), kadi.spades("K"), kadi.diamonds("K"), kadi.diamonds("Q")])).toBe(false);

        hand = [kadi.hearts("K"), kadi.spades("K"), kadi.diamonds("K"), kadi.diamonds("Q")];
        expect(kadi.RuleEngine.isValidMove(hand, kadi.joker('0'))).toBe(false);

        hand = [kadi.hearts("K"), kadi.spades("K"), kadi.clubs("K"), kadi.diamonds("K"), kadi.diamonds("Q")];
        expect(kadi.RuleEngine.evaluateGroup(hand)).toBe(true);
        
        expect(kadi.RuleEngine.isValidMove(hand, kadi.joker('0'))).toBe(true);
        expect(kadi.RuleEngine.isValidMove([kadi.clubs("8")], kadi.clubs("3"))).toBe(true);

        expect(kadi.RuleEngine.isValidMove([kadi.hearts("K"), kadi.clubs("K"), kadi.joker("0")], kadi.joker("1"))).toBe(true);
    });

    it("Assesses whether a move is valid", function() {
        var hand = [kadi.joker("0"), kadi.spades("2")];
        expect(kadi.RuleEngine.isValidMove(hand, kadi.diamonds("6"))).toBe(true);
        hand = [kadi.spades("2"),kadi.joker("0")];
        expect(kadi.RuleEngine.isValidMove(hand, kadi.diamonds("6"))).toBe(false);
    });

    it("Checks for valid moves", function() {
        var hand = [kadi.spades("Q"), kadi.joker("0")];
        expect(kadi.RuleEngine.isValidMove(hand, kadi.spades("5"))).toBe(true);
    });
    
    it("A Queen can follow a King if the previous number of kings after the first king is odd", function() {
        //K,Q
        expect(kadi.RuleEngine.canPlayTogetherWith(kadi.diamonds("K"), kadi.diamonds("Q"))).toBe(false);
        
        //K,K,Q
        expect(kadi.RuleEngine.canPlayTogetherWith(kadi.diamonds("K"), kadi.diamonds("Q"), [kadi.hearts("K")])).toBe(true);
       
        expect(kadi.RuleEngine.canPlayTogetherWith(kadi.diamonds("K"), kadi.diamonds("Q"), [kadi.hearts("K"), kadi.spades("K")])).toBe(false);
       
        expect(kadi.RuleEngine.canPlayTogetherWith(kadi.diamonds("K"), kadi.diamonds("Q"), [kadi.hearts("K"), kadi.spades("K"), kadi.clubs("K")])).toBe(true);
    });
    
    it("An ace can only follow a King if the previous card to the King is also a King", function() {
       expect(kadi.RuleEngine.canPlayTogetherWith(kadi.diamonds("K"), kadi.diamonds("A"))).toBe(false);
       
       expect(kadi.RuleEngine.canPlayTogetherWith(kadi.diamonds("K"), kadi.diamonds("A"), [kadi.hearts("K")])).toBe(true);
       
       expect(kadi.RuleEngine.canPlayTogetherWith(kadi.diamonds("K"), kadi.diamonds("A"), [kadi.hearts("K"), kadi.spades("K")])).toBe(false);
       
       expect(kadi.RuleEngine.canPlayTogetherWith(kadi.diamonds("K"), kadi.diamonds("A"), [kadi.hearts("K"), kadi.spades("K"), kadi.clubs("K")])).toBe(true);
       
    });

    it("A queen cannot finish a move", function() {
        var card = kadi.diamonds("Q");
        expect(kadi.RuleEngine.canEndMove(card)).toBe(false);

        card = kadi.diamonds("K");
        expect(kadi.RuleEngine.canEndMove(card)).toBe(true);
    ...
    describe("Queen rules", function() {
        ...
        it("A queen can play together with another queen", function() {
            ...
            expect(kadi.RuleEngine.canPlayTogetherWith(q_d, q_s)).toBe(true);
        });
        ...
        it("A queen can play together with another ordinary card of the same suite", function() {
            ...
            expect(kadi.RuleEngine.canPlayTogetherWith(q_d, seven_d)).toBe(true);
            expect(kadi.RuleEngine.canPlayTogetherWith(q_d, seven_c)).toBe(false);
        });
        ...
        it("An eight can act as a queen", function() {
            ...
            expect(kadi.RuleEngine.canPlayTogetherWith(eight_d, seven_d)).toBe(true);
        });
    });
    ...
    describe("Elimination rules", function() {
        ...
        it("The hand value is the sum of the elimination values of the cards in the hand", function() {
            ...
            expect(kadi.RuleEngine.calculateHandEliminationValue(hand)).toBe(61);

            hand = hand.concat([kadi.spades("3"), kadi.joker("0"), kadi.spades("A"), kadi.hearts("A")]);
            expect(kadi.RuleEngine.calculateHandEliminationValue(hand)).toBe(836);
        });
    });
    ...
    describe("Picking card rules", function() {
        ...
        it("An ace can block any picking card", function() {
            ...
            expect(kadi.RuleEngine.canBlock(h)).toBe(true);
        });

        it("An ace can answer any question", function() {
            expect(kadi.RuleEngine.canPlayTogetherWith(kadi.spades("Q"), kadi.hearts("A")));
        });

        it("An ordinary card cannot block a picking card", function() {
            var h = [kadi.diamonds("5")];
            expect(kadi.RuleEngine.canBlock(h)).toBe(false);
        });
        ...
        it("A picking card is a picking card", function() {
            expect(kadi.RuleEngine.canFollow(kadi.diamonds("3"), kadi.spades("2"))).toBe(true);
            expect(kadi.RuleEngine.canPlayTogetherWith(kadi.joker("0"), kadi.spades("2"))).toBe(true);
        });

        it("Returns the number of cards that can be used to block", function() {
            expect(kadi.RuleEngine.countBlockingCards([kadi.spades("A")])).toBe(1);
        });

        it("A picking move cannot mix a picking card and an ace", function() {
            var h = [kadi.diamonds("3"), kadi.spades("A")];
            expect(kadi.RuleEngine.canBlock(h)).toBe(true);
            expect(kadi.RuleEngine.isValidBlockingMove(h)).toBe(false);
            expect(kadi.RuleEngine.isValidBlockingMove([kadi.spades("A")])).toBe(true);
            expect(kadi.RuleEngine.isValidBlockingMove([kadi.spades("3"), kadi.clubs("2")])).toBe(true);
        });
    });
    ...
    describe("Ace rules", function() {
        ...
        it("The first card of a move is what determines whether play can proceed", function() {
            ...
            expect(kadi.RuleEngine.canFollowRequestedSuite(move,requested)).toBe(false);
            move = [kadi.spades("Q"),kadi.diamonds("A")];
            expect(kadi.RuleEngine.canFollowRequestedSuite(move,requested)).toBe(true);
        });
        ...
        it("Cards matching the requested suite", function() {
            ...
            expect(kadi.RuleEngine.cardsInHandMatchingSuite(hand,requested).length).toBe(2);
            expect(kadi.RuleEngine.canMeetMatchingSuite(hand,requested)).toBe(true);
        });

        it("All cards in hard match the requested suite if any suite is specified", function() {
            var req = kadi.Suite.ANY;
            var hand = [kadi.spades("A"), kadi.diamonds("A"), kadi.diamonds("K")];

            expect(kadi.RuleEngine.cardsInHandMatchingSuite(hand,req).length).toBe(3);
        });

        it("The best move matching the suite is the best group that can follow a requested card", function() {
            var requested = kadi.Suite.DIAMONDS;
    ...
    describe("KADI rules", function() {
        ...
        it("A player cannot be on kadi if they have any picking cards", function() {
            ...
            expect(kadi.RuleEngine.canDeclareKADI(hand)).toBe(false);
        });
        ...
        it("A player can be on kadi if they have a single ordinary card", function() {
            ...
            expect(kadi.RuleEngine.canDeclareKADI(hand)).toBe(true);
        });
        ...
        it("A player can finish with a single card", function() {
            var hand = [kadi.spades("4")];
            expect(kadi.RuleEngine.canFinish(hand, null, kadi.Suite.SPADES)).toBe(true);
        });
        ...
        it("A player can finish if the can form a move that matches the current top card", function() {
            ...
            expect(kadi.RuleEngine.canFinish(hand, topCard)).toBe(true);
        });
        ...
        it("A player cannot finish if the cant form a move that matches the current top card", function() {
            ...
            expect(kadi.RuleEngine.canFinish(hand, topCard)).toBe(false);
        });

        it("A player an finish if they can form a move that matches the request suite", function() {
            var hand = [kadi.spades("8"), kadi.diamonds("8"), kadi.diamonds("Q"), kadi.diamonds("4")];
            var requestedSuite = kadi.Suite.ANY;
            expect(kadi.RuleEngine.canFinish(hand, null, requestedSuite)).toBe(true);

            requestedSuite = kadi.Suite.HEARTS;
            expect(kadi.RuleEngine.canFinish(hand, null, requestedSuite)).toBe(false);
        });
        ...
        it("The number of entire moves possible is the number of moves that can follow the top card", function() {
            ...
            expect(kadi.RuleEngine.movesThatCanFollowTopCardOrSuite(hand, topCard).length).toBe(1);
        });
        ...
        it("The number of entire moves possible is the number of moves that can follow the requested suite", function() {
            ...
            var requestedSuite = kadi.Suite.HEARTS;

            expect(kadi.RuleEngine.movesThatCanFollowTopCardOrSuite(hand, null, requestedSuite).length).toBe(0);

            requestedSuite = kadi.Suite.SPADES;
            var moves = kadi.RuleEngine.movesThatCanFollowTopCardOrSuite(hand, null, requestedSuite);
        ...
        it("In single card KADI mode, you can only be only finish with one card", function() {
            ...
            expect(kadi.RuleEngine.canDeclareKADI(hand)).toBe(true);
            expect(kadi.RuleEngine.canDeclareKADI(hand,true)).toBe(false);

            hand = [kadi.spades("8")];
            expect(kadi.RuleEngine.canDeclareKADI(hand,true)).toBe(false);
            expect(kadi.RuleEngine.canDeclareKADI(hand,false)).toBe(false);

            hand = [kadi.spades("4")];
            expect(kadi.RuleEngine.canDeclareKADI(hand,true)).toBe(true);
            expect(kadi.RuleEngine.canDeclareKADI(hand,false)).toBe(true);
        });
    });
});
...
describe("Move rules:", function() {
    ...
    it("No moves are possible if no cards can follow", function() {
        ...
        expect(kadi.RuleEngine.possibleMoves(startingCard, hand).length).toBe(0);
    });
    ...
    it("Multiple moves are possible without grouping cards", function() {
        ...
        expect(kadi.RuleEngine.possibleMoves(kadi.diamonds("10"),hand).length).toBe(3);
    });
    ...
    describe("Group rules", function() {
        ...
        it("A group is a series of cards that an be played together", function() {
            ...
            expect(kadi.RuleEngine.evaluateGroup(g)).toBe(true);

            g = [kadi.clubs("7"), kadi.clubs("8")];
            expect(kadi.RuleEngine.evaluateGroup(g)).toBe(false);
        ...
        it("A group requires at least 2 cards in a hand", function() {
            var h = [kadi.clubs("3")];
            expect(kadi.RuleEngine.group(h, null).length).toBe(0);
        });
        ...
        it("A group exists if more than one cards in a hand can be played together", function () {
            ...
            expect(kadi.RuleEngine.group(h,tC).length).toBe(1);
        });
        ...
        it("A group with more cards is more valuable than one with less", function() {
            ...
            expect(kadi.RuleEngine.group(h,kadi.spades("6")).length).toBe(2);

            var group = kadi.RuleEngine.bestGroup(h,kadi.spades("6"));
            expect(group.length).toBe(3);
        });

        it("It can play selections", function() {
            var h = [kadi.diamonds("5"), kadi.spades("5")];
            expect(kadi.RuleEngine.canPlay(h, kadi.diamonds(9))).toBe(true);

            h = [kadi.hearts("K"), kadi.clubs("10"), kadi.spades("A")];
            expect(kadi.RuleEngine.canPlay(h, kadi.clubs("A"))).toBe(true);
        });
    });
});

describe("Game mechanics:", function() {
    var playerA = new kadi.Player('A', 'Chaos');
    var playerB = new kadi.Player('B', 'Player B', true);
    var playerC = new kadi.Player('C', 'Player C');
    var playerD = new kadi.Player('D', 'Player D');

    it("correctly formats the next player message", function() {
        var order = new kadi.PlayingOrder([playerA, playerB, playerC, playerD], 0);
    ...
    it("When anti-clockwise the index moves incrementally", function() {
        var order = new kadi.PlayingOrder([playerA, playerB, playerC, playerD], 0);
        order.direction = kadi.PlayingOrder.ANTI_CLOCKWISE;
        order.next();
    ...
    it("A single King causes a reverse action", function() {
        ...
        var h = [king];
        var action = kadi.RuleEngine.actionRequired(h);
        expect(action).toBe(kadi.RuleEngine.ACTION_REVERSE);
        expect(kadi.RuleEngine.calculateTurnsReverse(h)).toBe(1);
    });
    
    it("Two Kings cause a double reverse action", function() {
        
        var hand = [kadi.spades("K"), kadi.hearts("8"), kadi.diamonds("K")];
        var action = kadi.RuleEngine.actionRequired(hand);

        expect(action).toBe(kadi.RuleEngine.ACTION_REVERSE);
        expect(kadi.RuleEngine.calculateTurnsReverse(hand)).toBe(2);
    });

    it("Two jumps causes a double jump action", function() {
        var hand = [kadi.spades("J"), kadi.diamonds("J")];
        expect(kadi.RuleEngine.actionRequired(hand)).toBe(kadi.RuleEngine.ACTION_SKIP);

        expect(kadi.RuleEngine.calculateTurnsSkipped(hand)).toBe(2);
    });
    ...
    it("An ordinary card causes no action", function() {
        ...
        var h = [five];
        var action = kadi.RuleEngine.actionRequired(h);
        expect(action == kadi.RuleEngine.ACTION_NONE).toBe(true);
    });
    ...
    it("A picking card causes a picking or block action", function() {
        ...
        var h = [card];
        var action = kadi.RuleEngine.actionRequired(h);
        expect(action == kadi.RuleEngine.ACTION_PICK_OR_BLOCK).toBe(true);
    });

    it("An ace that isnt blocking a card causes a choose suite action", function() {
        var hand = [kadi.spades("Q"), kadi.spades("A")];
        var action = kadi.RuleEngine.actionRequired(hand);
        expect(action).toBe(kadi.RuleEngine.ACTION_PICK_SUITE);
    });
    ...
    it("The best suite to ask for is the one most common in my hand", function() {
        ...
        var suite = kadi.Strategy.askFor(hand);

        expect(suite).toBe(kadi.Suite.SPADES);

        hand = hand.concat([kadi.diamonds("Q"), kadi.diamonds("K")]);

        expect(kadi.Strategy.askFor(hand)).toBe(kadi.Suite.DIAMONDS);

        hand = hand.concat([kadi.spades("2")]);
        expect(kadi.Strategy.askFor(hand)).toBe(kadi.Suite.DIAMONDS);

        hand = [kadi.hearts("5")];
        expect(kadi.Strategy.askFor(hand)).toBe(kadi.Suite.HEARTS);

        hand = [kadi.joker("0")];
        expect(kadi.Strategy.askFor(hand)).toBe(kadi.Suite.ANY);
    });
    ...
    it("Two causes two cards to be picked", function() {
        ...
        expect(kadi.RuleEngine.calculatePicking(h)).toBe(2);
    });
    ...
    it("Three causes two cards to be picked", function() {
        ...
        expect(kadi.RuleEngine.calculatePicking(h)).toBe(3);
    });
    ...
    it("Joker causes 5 cards to be picked", function() {
        ...
        expect(kadi.RuleEngine.calculatePicking(h)).toBe(8);
    });

    it("When the game mode is to pick only the top card", function() {
        var h = [kadi.joker("0"), kadi.spades("2")];

        expect(kadi.RuleEngine.calculatePicking(h)).toBe(7);
        expect(kadi.RuleEngine.calculatePicking(h, false)).toBe(7);
        expect(kadi.RuleEngine.calculatePicking(h, true)).toBe(2);
    });
    ...
    it("A Question card causes an incomplete action", function() {
        ...
        var h = [card];
        var action = kadi.RuleEngine.actionRequired(h);
        expect(action == kadi.RuleEngine.ACTION_INCOMPLETE).toBe(true);
    });
    ...
    it("A Jack card causes a skip action", function() {
        ...
        var h = [card];
        var action = kadi.RuleEngine.actionRequired(h);
        expect(action == kadi.RuleEngine.ACTION_SKIP).toBe(true);
    });
