window.kadi.game = (function(me, $, undefined){
    ...
    me.Game = JS.Class({
        ...
        startGame: function() {
            ...
            SHOTGUN.listen(kadi.game.Events.INCREMENT_SKIP, function(player, card) {
                self.turnsToSkip++;
                card.active = false;
        ...
        progressPlay: function(player, action, playedCards, test) {
            ...
            _.delay(function() {
                if (action == kadi.game.RuleEngine.ACTION_NONE) {
                    ...
                    self.turnsToSkip = kadi.game.RuleEngine.calculateTurnsSkipped(playedCards);
                    ...
                    if (!canJump) {
                        ...
                        _.delay(function() {
                            ...
                            if(!test) {
                                SHOTGUN.fire(kadi.game.Events.MSG_RECEIVED, [ self.order.turn() ]);
                                SHOTGUN.fire(kadi.game.Events.RECEIVE_TURN,[self.tableDeck.topCard(), null],next.id);
                                SHOTGUN.fire(kadi.game.Events.RECEIVE_TURN,[next],'deck');
                            }
                            
                            SHOTGUN.fire(kadi.game.Events.RESET_PLAYER_CARDS, [], player.id);
        ...
        dealSpecificCards: function(playerCards, topCard) {
            ...
            SHOTGUN.fire(kadi.game.Events.RECEIVE_TURN,[this.tableDeck.topCard(),null,null],starter.id);
